...
#1.4 - fixed parameter declarations.  altered formatting on temp file cleanup.  run disk cleanup visibly if in an AttendedSession run.  fix typos.  add logic for resetbase to fail over to non-powershell if it doesnt have the -resetbase parameter.  removed an errant -whatif parameter.  fixed $AttendedRun logic because .IsPresent only works on Switch parameters and nothing else.  fixed some issues with dell command update, added in logic for a scenario where v3 is installed.
...
#Step 4 - Clear the CL Cache (yeah this isn't powershell code, there isn't actually powershell support for this?)
Write-Output "Clearing the CL Cache..."
Invoke-Command {certutil -URLcache * delete}

#Step 5 - Check and see if old versions of Windows still exist after an upgrade, and delete them if they do
if (Test-Path -Path "$Env:SystemDrive\Windows.old\") {
    Write-Output "Cleaning up C:\Windows.old..."
    Invoke-Command {takeown /F $Env:SystemDrive\Windows.old\* /R /A /D Y}
    Invoke-Command {cacls $Env:SystemDrive\Windows.old\*.* /C /T /grant administrators:F}
    Remove-Item -Force -Recurse "$Env:SystemDrive\Windows.old\" -ErrorAction SilentlyContinue
}
if (Test-Path -Path "$Env:SystemDrive\$Windows.~BT") {
    Write-Output "Cleaning up $Env:SystemDrive\$Windows.~BT..."
    Invoke-Command {takeown /F $Env:SystemDrive\$Windows.~BT\* /R /A /D Y}
    Invoke-Command {cacls $Env:SystemDrive\$Windows.~BT\*.* /C /T /grant administrators:F}
    Remove-Item -Force -Recurse "$Env:SystemDrive\$Windows.~BT\" -ErrorAction SilentlyContinue
}
if (Test-Path -Path "$Env:SystemDrive\$Windows.~WS") {
    Write-Output "Cleaning up $Env:SystemDrive\$Windows.~WS..."
    Invoke-Command {takeown /F $Env:SystemDrive\$Windows.~WS\* /R /A /D Y}
    Invoke-Command {cacls $Env:SystemDrive\$Windows.~WS\*.* /C /T /grant administrators:F}
    Remove-Item -Force -Recurse "$Env:SystemDrive\$Windows.~WS\" -ErrorAction SilentlyContinue
}

#Step 6 - Clean up system-wide stuff
...
Invoke-Command {reg import "$EnvSystemDrive\TuneUpReg.reg"} -ErrorAction SilentlyContinue #Far, far saner to import registry files using this than with powershell
try {
    if ([string]::IsNullOrEmpty($AttendedRun)) {
        ...
        Invoke-Command {C:\PsExec.exe -accepteula -s "cleanmgr" /sagerun:100} 
    } else {
...
if ($PCInfo.Manufacturer.Contains("Dell")) {
    ...
    if (Test-Path -Path "$Env:ProgramFiles\Dell\CommandUpdate\dcu-cli.exe") {
        ...
        Invoke-Command {C:\PsExec -acceptuela -s "$Env:ProgramFiles\Dell\CommandUpdate\dcu-cli.exe" /scan} -ErrorVariable ErrorVar
        if ([string]$ErrorVar -like "*error code 0*") {
            Write-Output "Installing Dell updates..."
            Invoke-Command {C:\PsExec -acceptuela -s "$Env:ProgramFiles\Dell\CommandUpdate\dcu-cli.exe" /applyUpdates}
        } else {
            ...
            if ([string]$ErrorVar -like "error code 2") {
                Write-Output "An unknown application error has occured."
                $ErrorCount += 1
            ...
            if ([string]$ErrorVar -like "error code 5") {
                Write-Output "Reboot pending from previous Command Update opeations"
                $ErrorCount += 1
        ...
        Invoke-Command {C:\PsExec.exe -accepteula -s "$Env:SystemDrive\DellCommandSetup.exe" /S /v /qn} -ErrorVariable ErrorVar
        if ($ErrorVar.Contains("error code 1602")) {
            ...
            Invoke-Command {C:\PsExec -acceptuela -s `"C:\Program Files\Dell\CommandUpdate\dcu-cli.exe`" /scan}
            Invoke-Command {C:\PsExec -acceptuela -s `"C:\Program Files\Dell\CommandUpdate\dcu-cli.exe`" /installUpdates}
        }
    }
    ...
    if ($PCInfo.Model.Contains("Surface")) {
        ...
        # Hyper-V VM
        Write-Output "This is a Hyper-V VM - it doesnt need driver updates."
    }
...
#STEP 8 - Check For And Force Repairs on Windows, Windows Image, and C drive
try {
...
if ($NoRebase.IsPresent) {
    Write-Ouput "Skipping OS rebase per your request..."
} else {
...
#STEP 12 - Application-Specific Cleanup
#I'M LOOKING AT YOU, TEAMS AND ADOBE!
Write-Output "Cleaning up Microsoft Teams's mess..."
...
if ($SkipAppUpdates.IsPresent) {
    Write-Output "Skipping application updates per your request..."
} else {
...
#STEP 14 - Windows Defender Operations
...
#STEP 15 - Clean up after ourselves and do a reboot
